...
def plot_fitness_over_generations(all_fitness_histories, algorithm, window=5, save_path="fitness_plot.png"):
    # Convert to numpy: shape = (num_runs, num_generations, pop_size)
    fitness_array = np.array(all_fitness_histories)

    # Mean & std across runs (axis=0 = runs, axis=2 = individuals)
    mean_fitness = fitness_array.mean(axis=2).mean(axis=0)
    std_fitness = fitness_array.mean(axis=2).std(axis=0)
    ...
    plt.plot(generations, mean_fitness, 'b-', label='Mean Fitness (3 runs)')
    plt.plot(ma_generations, ma_fitness, 'r-', label=f'Moving Avg')
...
def cmaes_algorithm(x0, sigma0, bounds):
    ...
    return fitness_history, best_genome


def genetic_algorithm(genome_size, bounds):
...
if __name__ == "__main__":
    ...
    print(f"RUNNING {ALGORITHM_TO_RUN} for 3 runs")

    for run in range(1):
        print(f"\nRUN {run+1}/3")
        if ALGORITHM_TO_RUN == 'CMAES_ALGO':
            fitness_history, best_genome = cmaes_algorithm(x0, sigma0, bounds)
        elif ALGORITHM_TO_RUN == 'GENETIC_ALGO':
            fitness_history, best_genome = genetic_algorithm(genome_size, bounds)
        all_fitness_histories.append(fitness_history)
        best_genomes.append(best_genome)

    # Visualize best genome of last run
    print("Visualizing best genome from last run...")
    controller(best_genomes[-1], visualize=True)

    # Plot averaged results
    plot_fitness_over_generations(all_fitness_histories, algorithm=ALGORITHM_TO_RUN, window=5)

    # Plot trajectory of best genome
    ...
    # Save
    np.savez("experiment_results_genetic.npz", 
