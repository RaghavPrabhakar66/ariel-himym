import mujoco
import numpy as np
from mujoco import viewer  # <-- NEW

from pathlib import Path   # <-- NEW: for auto-detecting latest CSV
...
class SpatialEA:
    ...
    def initialize_population(self) -> None:
        ...
        if config.incubation_enabled:
            ...
            # Demonstrate best incubation individual (this already opens a viewer)
    ...
    def mating_movement_phase(
        ...
        # Clear control callback so later viewers can set their own if needed
    ...
    # --------------------------------------------------------------
    # INTERACTIVE VIEWER FOR FINAL POPULATION (patched)
    # --------------------------------------------------------------
    def visualize_final_population(self, follow_core: bool = True) -> None:
        """
        Launch an interactive MuJoCo viewer showing the *current* population
        moving under the mating controller in the flat toroidal world.

        The viewer stays open until you close the window (ESC / close button).
        """

        # If evolution has finished and model/world were cleaned up, respawn.
        if self.model is None or self.data is None or self.world is None:
            print("[VIEWER] Model/world not initialized, spawning population...")
            self.spawn_population()
        else:
            # Make sure the latest positions/orientations are actually in the sim
            # (re-spawn at current_positions to avoid any drift from earlier runs)
            print("[VIEWER] Re-spawning robots at last known positions for visualization...")
            self.spawn_population()

        # Make sure we have tracked geoms for the current model
        if not self.tracked_geoms:
            self.tracked_geoms = get_tracked_geoms(
                world=self.world,
                data=self.data,
        ...
        # Build the same mating controller as in mating_movement_phase
        controller = create_mating_controller(
            population=self.population,
            tracked_geoms=self.tracked_geoms,
            num_joints=self.num_joints,
        ...
        # Register controller as mjcb_control so MuJoCo calls it every step
        mujoco.set_mjcb_control(controller)

        print("\n[VIEWER] Launching interactive viewer for final population.")
        print("         Close the window (ESC) to return to the terminal.\n")

        with viewer.launch_passive(self.model, self.data) as v:
            # Camera: top-down-ish above the toroidal arena
            v.cam.elevation = -70
            v.cam.azimuth = 90
            v.cam.distance = max(config.world_size[0], config.world_size[1]) * 1.5

            steps = 0
            while v.is_running():
                mujoco.mj_step(self.model, self.data)

                # Keep torus behaviour consistent with EA
                if config.use_periodic_boundaries:
                    apply_periodic_boundaries_to_simulation(
                        self.tracked_geoms,
                        (config.world_size[0], config.world_size[1])
                ...
                # Optionally follow the first robot's core geom
                if follow_core and self.tracked_geoms:
                    c = self.tracked_geoms[0].xpos
                    v.cam.lookat[:] = c

                v.sync()
                steps += 1

                # Light debug print ~once per simulated second
                if steps % int(1.0 / self.model.opt.timestep) == 0:
                    t = self.data.time
                    print(f"  [viewer] t={t:6.2f}  steps={steps}", end="\r")

        # Clean up controller callback so future runs don't reuse it accidentally
        mujoco.set_mjcb_control(None)

        print("\n[VIEWER] Window closed, returning to program.\n")
    ...
    def run_evolution(self) -> SpatialIndividual | None:
        ...
        for gen in range(self.num_generations):
            ...
            if config.stop_on_limits:
                if self.population_size >= config.max_population_limit:
                    ...
                    print(f"{'!' * 60}")
                    print(
                        f"Population size ({self.population_size}) reached or exceeded maximum limit ({config.max_population_limit})")
                ...
                if self.population_size < config.min_population_limit:
                    ...
                    print(f"{'!' * 60}")
                    print(
                        f"Population size ({self.population_size}) dropped below minimum limit ({config.min_population_limit})")
                    print(f"Stopping evolution at generation {gen}")
                    print(f"{'!' * 60}")
            ...
            if gen > 0:  # Skip selection in first generation (no offspring yet)
                ...
                if self.population:
                    ...
                    print(
                        f"  Pre-selection fitness range: {min(fitness_values_pre_selection):.4f} to {max(fitness_values_pre_selection):.4f}")
        ...
        # NOTE: we don't rely on a return value here; main() will scan the
        # results folder for the newest CSV when constructing ExperimentVisualizer.
...
def main():
    ...
    # Auto-detect the latest evolution CSV in results folder
    results_dir = Path(config.results_folder)
    csv_files = sorted(results_dir.glob("evolution_data_*.csv"))
    if csv_files:
        latest_csv = str(csv_files[-1])
        print(f"\n[REPORT] Using latest CSV for analysis: {latest_csv}")
        viz = ExperimentVisualizer(latest_csv)
        viz.generate_report()
    else:
        print("\n[REPORT] No evolution_data_*.csv found in results folder; skipping ExperimentVisualizer.")

    # Interactive visualization of the final population on the torus.
    # This will open a viewer and will NOT close automatically.
    spatial_ea.visualize_final_population()
