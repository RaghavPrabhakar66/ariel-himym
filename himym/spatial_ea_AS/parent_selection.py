...
def _mating_zone_pairing(
    ...
    # Now pair robots that are in a mating zone using nearest-neighbor,
    # making sure each robot appears in at most one pair.
    for idx in robots_in_zone:
        # Skip if already paired in a previous step
        if idx in paired_indices:
            continue
