...
def evaluate_individual_cost(individual: List[float], duration: float = 20.0, trials: int = EVAL_TRIALS, seed_base: int = 0) -> Tuple[float]:
    ...
    def single_run(world_ctor, seed_offset):
        ...
        flip_penalty = float(np.any(z_values < 0.05)) * 1.0

        # ---------------------------
        # Adjusted cost formula
        # ---------------------------
        forward_reward_weight = 10.0  # strongly encourage forward motion
        lateral_penalty_weight = 0.1  # low initial penalty
        straight_penalty_weight = 0.1
        energy_weight = 0.001
        flip_weight = 100.0

        cost = -forward_reward_weight * forward \
               + lateral_penalty_weight * lateral \
               + straight_penalty_weight * straightness_penalty \
    ...
    world_constructors = [SimpleFlatWorld]
